...
namespace Cassandra.IntegrationTests.Core
{
    ...
    public class ConsistencyTests : TestGlobals
    {
        ...
        private MappingConfiguration _config = new MappingConfiguration().Define(new ManyDataTypesPocoMappingCaseSensitive());
        private Table<ManyDataTypesPoco> _table;
        private string _defaultSelectStatement = "SELECT * FROM \"" + typeof(ManyDataTypesPoco).Name + "\"";
        private List<ManyDataTypesPoco> _defaultPocoList;
        ...
        private ITestCluster SetupSessionAndCluster(int nodes, Dictionary<string, string> replication = null)
        {
            ...
            // Setup unique table
            _table = new Table<ManyDataTypesPoco>(_session, _config);
            _table.Create();
            _defaultPocoList = ManyDataTypesPoco.GetDefaultAllDataTypesList();
        ...
        public void Consistency_SimpleStatement_LocalSerial_Insert_Success()
        {
            ...
            SimpleStatement simpleStatement = _table.Insert(ManyDataTypesPoco.GetRandomInstance());
        ...
        [Test, Category("large")]
        public void Consistency_SimpleStatement_Three_Select()
        {
            int copies = 3;
        ...
        [Test, Category("large")]
        public void Consistency_SimpleStatement_Three_Insert()
        {
            int copies = 3;
            Dictionary<string, string> replication = new Dictionary<string, string> { { "class", ReplicationStrategies.SimpleStrategy }, { "replication_factor", copies.ToString() } };
            var testCluster = SetupSessionAndCluster(copies, replication);
        ...
        [Test, Category("large")]
        public void Consistency_SimpleStatement_Two_Select()
        {
            int copies = 2;
            Dictionary<string, string> replication = new Dictionary<string, string> { { "class", ReplicationStrategies.SimpleStrategy }, { "replication_factor", copies.ToString() } };
            var testCluster = SetupSessionAndCluster(copies, replication);
        ...
        [Test, Category("large")]
        public void Consistency_SimpleStatement_Two_Insert()
        {
            int copies = 2;
            Dictionary<string, string> replication = new Dictionary<string, string> { { "class", ReplicationStrategies.SimpleStrategy }, { "replication_factor", copies.ToString() } };
            var testCluster = SetupSessionAndCluster(copies, replication);
        ...
        {
            ManyDataTypesPoco mdtp = ManyDataTypesPoco.GetRandomInstance();
            object[] vals = GetObjectArrayFromManyDataTypesPocoForPreparedInsert(mdtp);
            SetupSessionAndCluster(_defaultNodeCountOne);

            string preparedStatementAsString = _table.Insert(new ManyDataTypesPoco()).ToString();
            PreparedStatement preparedInsertStatement = _session.Prepare(preparedStatementAsString);
        ...
        [Test, NUnit.Framework.Ignore("Pending question: no SetSerialConsistencyLevel for prepared statements ?")]
        ...
        [Test, Category("large")]
        public void Consistency_PreparedStatement_Three_Select()
        {
            int copies = 3;
            Dictionary<string, string> replication = new Dictionary<string, string> { { "class", ReplicationStrategies.SimpleStrategy }, { "replication_factor", copies.ToString() } };
            var testCluster = SetupSessionAndCluster(copies, replication);
        ...
        [Test, Category("large")]
        public void Consistency_PreparedStatement_Three_Insert()
        {
            int copies = 3;
            Dictionary<string, string> replication = new Dictionary<string, string> { { "class", ReplicationStrategies.SimpleStrategy }, { "replication_factor", copies.ToString() } };
            var testCluster = SetupSessionAndCluster(copies, replication);
        ...
        [Test, Category("large")]
        public void Consistency_PreparedStatement_Two_Select()
        {
            int copies = 2;
            Dictionary<string, string> replication = new Dictionary<string, string> { { "class", ReplicationStrategies.SimpleStrategy }, { "replication_factor", copies.ToString() } };
            var testCluster = SetupSessionAndCluster(copies, replication);
        ...
        [Test, Category("large")]
        public void Consistency_PreparedStatement_Two_Insert()
        {
            int copies = 2;
            Dictionary<string, string> replication = new Dictionary<string, string> { { "class", ReplicationStrategies.SimpleStrategy }, { "replication_factor", copies.ToString() } };
            var testCluster = SetupSessionAndCluster(copies, replication);
        ...
        public void Consistency_Batch_Two()
        {
            int copies = 2;
            Dictionary<string, string> replication = new Dictionary<string, string> { { "class", ReplicationStrategies.SimpleStrategy }, { "replication_factor", copies.ToString() } };
            SetupSessionAndCluster(_defaultNodeCountOne, replication);
        ...
        private void DoBatchInsertTest(ConsistencyLevel expectedConsistencyLevel)
        {
            ...
            var addlPocoList = ManyDataTypesPoco.GetDefaultAllDataTypesList();
        ...
        private void DoPreparedInsertTest(ConsistencyLevel expectedConsistencyLevel)
        {
            ...
            // NOTE: We have to re-prepare every time since there is a unique Keyspace used for every test
            string preparedStatementAsString = _table.Insert(new ManyDataTypesPoco()).ToString();
            PreparedStatement preparedInsertStatement = _session.Prepare(preparedStatementAsString).SetConsistencyLevel(expectedConsistencyLevel);
        ...
        private static object[] GetObjectArrayFromManyDataTypesPocoForPreparedInsert(ManyDataTypesPoco mdtp)
        {
            ...
            object[] vals =  
            {
                ...
                mdtp.TimeUuidType, mdtp.NullableTimeUuidType, mdtp.DictionaryStringLongType, mdtp.DictionaryStringStringType, mdtp.ListOfGuidsType, mdtp.ListOfStringsType,
            };
